...
// Supports account rotation on 429/402 responses when a rotator is configured.
func (s *Server) handleMessages(w http.ResponseWriter, r *http.Request) {
        ...
        if rot != nil && rot.AutoRotate() {
                s.handleMessagesWithRotation(w, r, req, rot)
                return
        }

        resp, model, stream, hardErr, status := s.doMessagesOnce(r.Context(), req)
        if hardErr != nil {
                writeAnthropicError(w, http.StatusBadGateway, "api_error", hardErr.Error())
                return
        }
        defer resp.Body.Close()
        if status >= 400 {
                b, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
                writeAnthropicError(w, status, "api_error", string(b))
                return
        }
        s.flushMessagesResponse(w, r, resp, model, stream)
}

// doMessagesOnce performs a single Anthropic->OpenAI translated upstream call.
// Returns the open response (caller closes), the resolved model, whether
// streaming was requested, any transport error, and the HTTP status code.
func (s *Server) doMessagesOnce(
        ctx context.Context,
        req map[string]any,
) (resp *http.Response, model string, stream bool, hardErr error, status int) {
        token, _, settings, err := s.ensure(ctx)
        if err != nil {
        ...
        upReq, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(raw))
        if err != nil {
                return nil, model, stream, err, 0
        }
        ...
        return resp, model, stream, nil, status
}

// flushMessagesResponse writes a successful upstream response back to the
// Anthropic client, translating OpenAI chat format to Anthropic Messages
// format (stream or non-stream).
func (s *Server) flushMessagesResponse(w http.ResponseWriter, r *http.Request, resp *http.Response, model string, stream bool) {
        defer resp.Body.Close()
        if !stream {
...
// handleMessagesWithRotation runs the Anthropic handler through the rotator.
// 429/402 responses trigger account rotation; non-rotatable errors and
// successes are flushed to the client after the rotation loop completes.
func (s *Server) handleMessagesWithRotation(w http.ResponseWriter, r *http.Request, req map[string]any, rot *rotator.Rotator) {
        ctx := r.Context()
        var captured *capturedMessages
        var lastStatus int
        finalErr := rot.RunWithRotation(ctx, func(ctx context.Context, accountID string) (rotator.Result, error) {
                resp, model, stream, hardErr, status := s.doMessagesOnce(ctx, req)
                lastStatus = status
                if hardErr != nil {
                        if resp != nil {
                                resp.Body.Close()
                ...
                captured = &capturedMessages{resp: resp, model: model, stream: stream, status: status}
                return rotator.Result{}, nil
        })
        if captured != nil && captured.resp != nil {
                if captured.status >= 400 {
                        b, _ := io.ReadAll(io.LimitReader(captured.resp.Body, 1<<20))
                        writeAnthropicError(w, captured.status, "api_error", string(b))
                        return
                }
                s.flushMessagesResponse(w, r, captured.resp, captured.model, captured.stream)
                return
        }
        if finalErr != nil {
                writeAnthropicError(w, http.StatusBadGateway, "api_error", fmt.Sprintf("%s (last_status=%d)", finalErr.Error(), lastStatus))
                return
        }
        writeAnthropicError(w, http.StatusBadGateway, "api_error", "rotator returned no response")
}

// capturedMessages holds a buffered upstream response for the Anthropic path.
