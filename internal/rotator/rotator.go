...
type Rotator struct {
        ...
        // autoRegFn, when set, is called when all accounts are limited. It should
        // create & save a fresh account and return its ID. Returning an error
        // makes the rotator give up the rotation loop.
        autoRegFn      func(ctx context.Context) (string, error)
        autoRegEnabled bool
        // autoRegMu serializes invocations of autoRegFn across concurrent goroutines.
...
// SetAutoReg wires a callback that creates a fresh account on-demand. When
// enabled via SetAutoRegEnabled(true), the rotator will call this function
// the first time all configured accounts are limited, instead of failing the
// request. The callback should block until the new account is saved to the
// store and return its ID.
func (r *Rotator) SetAutoReg(fn func(ctx context.Context) (string, error)) {
        r.mu.Lock()
        defer r.mu.Unlock()
        r.autoRegFn = fn
}

// SetAutoRegEnabled toggles whether the auto-register callback is invoked.
// Even if SetAutoReg was called, the callback is only fired when enabled.
func (r *Rotator) SetAutoRegEnabled(v bool) {
        r.mu.Lock()
        defer r.mu.Unlock()
        r.autoRegEnabled = v
}

// AutoRegEnabled returns whether auto-registration is currently enabled.
func (r *Rotator) AutoRegEnabled() bool {
        r.mu.Lock()
        defer r.mu.Unlock()
        return r.autoRegEnabled
}
        ...
        for attempt := 0; attempt < maxAttempts; attempt++ {
                ...
                if err != nil {
                        ...
                        if r.AutoRotate() && IsLimitError(err) {
                                ...
                                // Last resort: auto-register a fresh account on the fly.
                                if newID, ok := r.tryAutoReg(ctx); ok {
                                        if r.verbose {
                                                fmt.Printf("[rotator] auto-registrou nova conta %s\n", shortID(newID))
                                        }
                                        current = newID
                                        continue
                                }
...
// tryAutoReg invokes the auto-register callback if one is configured and
// enabled. It serializes concurrent calls so that only one account is created
// at a time even under burst load.
func (r *Rotator) tryAutoReg(ctx context.Context) (string, bool) {
        r.mu.Lock()
        fn := r.autoRegFn
        enabled := r.autoRegEnabled
        r.mu.Unlock()
        if fn == nil || !enabled {
                return "", false
        }
        // Use a separate lock to serialize auto-reg invocations across goroutines.
        r.autoRegMu.Lock()
        defer r.autoRegMu.Unlock()
        id, err := fn(ctx)
        if err != nil || id == "" {
                return "", false
        }
